...
describe('cache', () => {
  ...
  describe('cacher', () => {
    ...
    it('stores and retrieves packuments and peers, persisting across instances', async () => {
      ...
      cache!.set('foo', packument)
      expect(cache!.get('foo')).toStrictEqual(packument)
      ...
      expect(reloaded!.get('foo')).toStrictEqual(packument)
    })

    it('logs the number of cache hits', async () => {
      const logSpy = vi.spyOn(console, 'log').mockImplementation(() => {})
      const cache = await cacher({ cache: true, cacheFile })
      cache!.set('foo', createMockVersion({ name: 'foo', versions: { '1.2.3': '' } }))
      cache!.get('foo')
      cache!.log()
