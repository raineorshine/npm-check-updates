...
describe('printError', () => {
  ...
  it('strips terminal escape sequences from an error message', async () => {
    const output = await captureRawStderr(() => printError(new Error('E404 Not Found' + OSC_TITLE)))
    expect(output).toContain('E404 Not Found')
    expect(output).not.toContain('pwned')
  })

  it('strips terminal escape sequences from a non-Error value', async () => {
    const output = await captureRawStderr(() => printError('boom' + OSC_TITLE))
    expect(output).toContain('boom')
    expect(output).not.toContain('pwned')
  })
})

describe('printErrorVerbose', () => {
  chalkInit(false)

  it('strips terminal escape sequences from the caught error', async () => {
    const output = await captureRaw(() =>
      printErrorVerbose({ loglevel: 'verbose' }, 'Failed to get the peer dependencies of a@1.0.0', 'boom' + OSC_TITLE),
    )
