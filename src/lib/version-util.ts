...
const WILDCARD_PURE_REGEX = new RegExp(`^(${WILDCARDS_PURE.map(wildcard => escapeRegExp(wildcard)).join('|')})$`)
  ...
  function chooseVersion(part: VersionPart): string | null {
    return (
      (isWildPart(declaredSemver[part])
        ? declaredSemver[part]
        : (VERSION_BASE_PARTS.includes(part) && declaredSemver[part]) || VERSION_ADDED_PARTS.includes(part)
          ? latestSemver[part]
          : null) || null
    )
  }
