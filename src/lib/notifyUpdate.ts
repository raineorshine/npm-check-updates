...
const cacheFile = path.join(os.homedir(), '.ncu-update-check.json')
...
function renderNotification(current: string, latest: string): string {
  ...
  let latestColored = style.green(latest)
  if (type === 'major') {
    latestColored = style.red(latest)
  } else if (type === 'minor') {
    latestColored = style.yellow(latest)
  }

  const message = `Update available ${style.dim(current)}${style.reset(' → ')}${latestColored}
Run ${style.cyan('npm i -g npm-check-updates')} to update
${style.dim.underline(type === 'major' ? releaseUrls.map(url => style.dim.underline(url)).join('\n') : compareUrl)}`
...
async function refreshCache(): Promise<void> {
  try {
    ...
    await writeFile(cacheFile, JSON.stringify({ lastUpdateCheck: Date.now(), latest }, null, 2), { mode: 0o600 })
  } catch {
