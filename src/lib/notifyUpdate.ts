...
function renderNotification(update: UpdateInfo): string {
  ...
  const message = `Update available ${chalk.dim(update.current)}${chalk.reset(' → ')}${
    update.type === 'major'
      ? chalk.red(update.latest)
      : update.type === 'minor'
        ? chalk.yellow(update.latest)
        : chalk.green(update.latest)
  }
Run ${chalk.cyan(`npm i -g ${pkg.name}`)} to update
${chalk.dim.underline(
  update.type === 'major' ? releaseUrls.map(url => chalk.dim.underline(url)).join('\n') : compareUrl,
)}`
