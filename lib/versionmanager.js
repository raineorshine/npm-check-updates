...
// track if all packages return error 404
let allNotFoundErrors;
...
function queryVersions(packageMap, options = {}) {
    ...
    function getPackageVersionProtected(dep) {
        return getPackageVersion(dep, packageMap[dep], options.pre).catch(err => {
            ...
            if (result !== null) {
                allNotFoundErrors = false;
            } else {
                allNotFoundErrors = true;
            }
            if (bar) {
                bar.tick();
            }
...
/**
 * Check if it's necesary to test the internet conncetion
 * @param {boolean} noOnlineCheck
 * @returns {boolean}
 */
function checkInternetConnection(noOnlineCheck) {
    if (!noOnlineCheck && allNotFoundErrors === true) {
        return true;
    } else {
        return false;
    }
