...
function upgradeDependencyDeclaration(declaration, latestVersion, options) {
    ...
    */
    ...
    var newSemver = cint.toObject(versionUtil.VERSION_PARTS, part => {
...
function upgradePackageDefinitions(currentDependencies, options) {
    ...
    return queryVersions(currentDependencies, {
        ...
        var filteredUpgradedDependencies = _.pickBy(upgradedDependencies, (v, dep) => {
...
function queryVersions(packageMap, options) {
    ...
    var packageManager = getPackageManager(options.packageManager);
...
function getPreferredWildcard(dependencies) {
    ...
    var groups = _.groupBy(_.values(dependencies), dep => {
        return _.find(versionUtil.WILDCARDS, wildcard => {
    ...
    var arrOfGroups = cint.toArray(groups, (wildcard, instances) => {
        return {
            wildcard: wildcard,
            instances: instances
        };
    ...
    var sorted = _.sortBy(arrOfGroups, wildcardObject => {
