...
function getCurrentDependencies(packageFile, callback) {
    readJson(packageFile, null, false, function(error, json) {
        //Handle error and no result
        if (error !== null || json === null)
            return callback(error, json);

        //Unify dev and classic dependencies
        var allDependencies = mergeObjects(json.dependencies, json.devDependencies);

        //Remove no npm packages
        var filteredDependencies = filterObjects(allDependencies, function(item) {
            return item.indexOf('http') === -1 && item.indexOf('ssh') === -1;
        });

        callback(error, filteredDependencies);
    });
}
...
function filterObjects(o, filter) {
    var newObject = {};

    for (var property in o) {
        if (filter(o[property]))
