...
function upgradeDependencies(currentDependencies, latestVersions) {
    var upgradedDeps = {};
    for (var packageName in currentDependencies) {
        var current = currentDependencies[packageName];
        var latest = latestVersions[packageName];
        if(isUpgradeable(current, latest)) {
            upgradedDeps[packageName] = upgradeDependencyDeclaration(current, latest);
        }
    }
