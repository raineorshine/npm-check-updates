...
function upgradeDependencies(currentDependencies, latestVersions, options) {
    ...
    options.keepSemver = options.keepSemver || false;

    if (options.keepSemver) {
        Object.keys(currentDependencies).forEach(key => {
            var currentVersion = currentDependencies[key];
            var latestVersion = latestVersions[key];

            var clean = function(version) {
                return version.replace(/(<|>|=)*/g, '').trim();
            }

            var semverSatisfies = semver.satisfies(latestVersion, currentVersion)
            if (!semverSatisfies) {
                latestVersions[key] = clean(currentVersion);
            }
        })
    }
