...
function upgradeDependencyDeclaration(declaration, latestVersion, options) {
    ...
    */
    ...
    const newSemver = cint.toObject(versionUtil.VERSION_PARTS, function (part) {
...
function upgradePackageDefinitions(currentDependencies, options) {
    ...
    return queryVersions(currentDependencies, {
        ...
        const filteredUpgradedDependencies = _.pickBy(upgradedDependencies, function (v, dep) {
...
function getCurrentDependencies(pkgData, options) {
    ...
    // if no specific dependency section, is requested, enable prod, dev, and optional by default
    if (!options.prod && !options.dev && !options.optional && !options.peer) {
...
function getInstalledPackages(options) {
    ...
    return getPackageManager(options.packageManager).list({cwd: options.cwd, prefix: options.prefix, global: options.global}).then(function (pkgInfoObj) {
        if (!pkgInfoObj) {
        ...
        return cint.filterObject(pkgInfoObj, function (dep, version) {
            return version && !versionUtil.isWildPart(version) && filterFunction(dep);
        });
    });
}
...
function queryVersions(packageMap, options) {
    ...
    function getPackageVersionProtected(dep) {
        return getPackageVersion(dep, packageMap[dep], options.pre).catch(function (err) {
            if (err && err.match && err.match(/404 Not Found/i)) {
                return null;
            } else {
                throw err;
            }
...
function getPreferredWildcard(dependencies) {
    ...
    const groups = _.groupBy(_.values(dependencies), function (dep) {
    ...
    const arrOfGroups = cint.toArray(groups, function (wildcard, instances) {
        return {
            wildcard,
            instances
        };
    ...
    const sorted = _.sortBy(arrOfGroups, function (wildcardObject) {
