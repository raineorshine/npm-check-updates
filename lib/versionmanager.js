 ...
 * (now part of npm-check-updates/upgradePackageDefinitions, in order to access upgradeTable)
 * (but doesn't that reduce test coverage?)
 *
 * Get the latest versions from the NPM repository
 * @param packageList               A list of package names to query
 * @param options.versionTarget     'latest' or 'greatest' (Default: latest)
 * @param options.data              Callback that is fired whenever a package version request resolves or rejects.
 * @returns                         Promised {packageName: version} collection
 */
        ...
        .then(function (versions) {
            return Promise.map(versions, function (version) {
                if (version.isRejected()) {
                    ...
                    // ignore 404's
                    if (reason.message == 404) { // eslint-disable-line eqeqeq
                        return null;
                    } else {
