...
function getCurrentDependencies(pkg, options) {
    ...
    var allDependencies = cint.filterObject(_.merge({},
        ...
        options.filter ? packageNameFilter(options.filter) : _.identity,
        options.reject ? _.negate(packageNameFilter(options.reject)) : _.identity
    ));
...
function getInstalledPackages(options) {
    ...
    return selectedPackageManager.list().then(function (results) {
        ...
        var filterFunction = packageNameFilter(options.filter);
