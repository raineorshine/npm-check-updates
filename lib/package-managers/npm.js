...
async function viewMany(packageName, fields, currentVersion, { timeout } = {}) {
  ...
  const result = await pacote.packument(packageName, { ...npmConfig, timeout })
...
module.exports = {
  ...
  async newest(packageName, currentVersion, options = {}) {
    ...
    return _.last(
      filterOutPrereleaseVersions(versionsWithTime, options.pre !== false)
    )
  },
  ...
  async greatest(packageName, currentVersion, options = {}) {
    const versions = await viewOne(packageName, 'versions', currentVersion, { timeout: options.timeout })
