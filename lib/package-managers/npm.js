...
const npmConfig = Object.assign({}, require('libnpmconfig').read());
...
function view(packageName, field, currentVersion) {
    ...
    npmConfig.registry = registryUrl(scope).toString();
    const authInfo = registryAuthToken(npmConfig.registry, {recursive: true});
    if (authInfo) {
        if (authInfo.type === 'Bearer') {
            npmConfig.token = authInfo.token;
        } else if (authInfo.type === 'Basic') {
            if (authInfo.username && authInfo.password) {
                npmConfig.username = authInfo.username;
                npmConfig.password = authInfo.password;
            } else {
                npmConfig._auth = authInfo.token;
            }
        }
    } else {
        delete npmConfig.token;
        delete npmConfig.username;
        delete npmConfig.password;
        delete npmConfig._auth;
    }

    if (field === 'time') {
        npmConfig['full-metadata'] = true;
    }
