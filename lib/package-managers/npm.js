...
async function packageAuthorChanged(packageName, currentVersion, latestVersion) {
    ...
    const current = versionUtil.isWildCard(currentVersion.charAt(0)) ? currentVersion.substr(1) : currentVersion;
    const latest = versionUtil.isWildCard(latestVersion.charAt(0)) ? latestVersion.substr(1) : latestVersion;

    const result = await pacote.packument(packageName, npmConfig);
    if (result.versions) {
        const currentData = result.versions[current];
        const latestData = result.versions[latest];
        const currentAuthor = currentData && currentData.author ? currentData.author : null;
        const latestAuthor = latestData && latestData.author ? latestData.author : null;
