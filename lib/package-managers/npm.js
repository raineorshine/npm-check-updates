    ...
    */
    ...
    newest: function (packageName, currentVersion, pre) {
        return view(packageName, 'time')
            ...
            .then(function (versions) {
                return pre ? _.last(versions) : filterOutPrereleaseVersions(versions);
            });
    },
    ...
    greatestMajor: function (packageName, currentVersion, pre) {
        return view(packageName, 'versions').then(function (versions) {
            return (
                pre ?
                versionUtil.findGreatestByLevel(versions, currentVersion, 'major') :
                filterOutPrereleaseVersions(versions)
            );
        });
    },
    ...
    greatestMinor: function (packageName, currentVersion, pre) {
        return view(packageName, 'versions').then(function (versions) {
            return (
                pre ?
                versionUtil.findGreatestByLevel(versions, currentVersion, 'minor') :
                filterOutPrereleaseVersions(versions)
            );
        });
    }
