...
module.exports = {
    ...
    */
    ...
    latest: function (packageName, currentVersion, includePrerelease) {
            ...
            .then(function (version) {
                if (
                    ...
                    return view(packageName, 'versions')
                        .then(versionUtil.filterOutPrereleaseVersions)
                        .then(_.last);
                }
            });
    },

    newest: function (packageName, currentVersion, includePrerelease) {
        return view(packageName, 'time')
            .then(_.keys)
            .then(function (versions) {
                if (!includePrerelease) {
                    versions = versionUtil.filterOutPrereleaseVersions(versions);
                }
                return _.last(versions);
            });
    },

    greatest: function (packageName, currentVersion, includePrerelease) {
        return view(packageName, 'versions')
            .then(function (versions) {
                if (!includePrerelease) {
                    versions = versionUtil.filterOutPrereleaseVersions(versions);
    ...
    greatestMajor: function (packageName, currentVersion, includePrerelease) {
    ...
    greatestMinor: function (packageName, currentVersion, includePrerelease) {
        return view(packageName, 'versions').then(function (versions) {
            if (!includePrerelease) {
                versions = versionUtil.filterOutPrereleaseVersions(versions);
            }
