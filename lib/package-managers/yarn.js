...
async function defaultPrefix(options) {
  ...
  // FIX: catch error to prevent from process to crash with yarn v2
  // https://github.com/raineorshine/npm-check-updates/issues/873
  let prefix
  try {
    prefix = await spawn(cmd, ['global', 'dir'])
  } catch (err) {}
...
module.exports = {
  ...
  async latest(packageName, currentVersion, options = {}) {
    const latest = await viewOne(packageName, 'dist-tags.latest', currentVersion, options)
  ...
  greatest(packageName, currentVersion, options = {}) {
    return viewOne(packageName, 'versions', currentVersion, options).then(versions =>
