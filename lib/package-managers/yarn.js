...
function defaultPrefix(options) {
  ...
  return spawn(cmd, ['global', 'dir']).then(prefix => {
    ...
    // https://github.com/raineorshine/yarn-check-updates/issues/146

    return options.global && prefix.match('Cellar') ? '/usr/local' :

      // Workaround: get prefix on windows for global packages
      // Only needed when using yarn api directly
      process.platform === 'win32' && options.global && !process.env.prefix ?
        prefix ? prefix.trim() : `${process.env.APPDATA}\\Yarn\\Data\\global` :
        null
  })
}
