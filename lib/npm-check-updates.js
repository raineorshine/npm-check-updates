    ...
    .then(globalPackages => {
      ...
      return vm.upgradePackageDefinitions(globalPackages, options)
        .then(([upgraded, latest]) => {
          ...
          if (upgraded) {
            // Here I think it is more appropriate to change to latest.
            // In addition, I have a little doubt whether it is appropriate to include npm itself?
            // Because some software is still using the old node version.
            instruction = upgradedPackageNames.map(pkg => pkg + '@latest').join(' ')
