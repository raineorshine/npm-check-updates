...
function toDependencyTable(args, options) {
    ...
    const rows = Object.keys(args.to).map(dep => {
        ...
        const owner = options.ownerChanged && args.owner && args.owner[dep] ? '*owner changed*' : '';
 ...
 * @param {Object} args.owners - Boolean flag per dependency which announces if package owner changed.
 * @returns {void}
 */
