    ...
    return readJson(packageFile, null, false)
        ...
        .spread(function (current, installed, upgraded, latest) {
            ...
            if (!_.isEmpty(upgraded)) {
                if (program.upgrade) {
                    upgradePackageFile(packageFile, current, upgraded)
                        .then(function () {
    ...
    if (_.isEmpty(upgraded)) {
        ...
        for (var dep in upgraded) {
            print('"' + dep + '" can be updated from ' +
                ...
                " (Installed: " + (installed[dep] ? installed[dep] : "none") + ", " + superlative + ": " + latest[dep] + ")");
        }
    }
}
...
vm.initialize(program.global).then(function () {
    ...
    if (program.global) {
        ...
        return analyzeProjectDependencies(packageFile);
    }
})
