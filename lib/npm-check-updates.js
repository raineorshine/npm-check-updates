...
function analyzeProjectDependencies(pkgData, pkgFile) {
    ...
    .spread(function (current, installed, upgraded, latest) {
        ...
        if(program.json || program.jsonFlat) {
            ...
            print(program.json ?
                _.pick(JSON.parse(newPkgData), 'dependencies', 'devDependencies') : 
                upgraded
            );
        }
        else {
...
program
    ...
    .option('-j, --jsonAll', 'output upgraded package.json instead of human-readable upgrade messages')
    .option('--jsonDeps', 'output upgraded just the dependencies and devDependencies in json')
    .option('--jsonDepsFlat', 'output upgraded dependencies in flat json (deps and devDeps not separated)')
