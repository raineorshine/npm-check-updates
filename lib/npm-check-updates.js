...
async function findPackage(options) {
    ...
    if (options.packageData) {
        ...
        const stdinData = getstdin().then(data => {
        ...
        pkgFile = stdinData.then(data => data ? null : findUp.sync(pkgFileName));
        pkgData = stdinData.then(async data => data || getPackageDataFromFile(await pkgFile, pkgFileName));
    } else {
