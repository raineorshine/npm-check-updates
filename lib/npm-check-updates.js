 ...
 */
async function getOwnerPerDependency(args, options) {
    const packageManager = vm.getPackageManager(options.packageManager);
    args.owner = {};
    await Promise.all(
        Object.keys(args.to).map(async (dep) => {
            const from = args.from[dep] || null;
            const to = args.to[dep] || null;
            const ownerChanged = await packageManager.packageAuthorChanged(dep, from, to);
            // eslint-disable-next-line require-atomic-updates
            args.owner[dep] = ownerChanged;
        })
    );
    return args;
}
 ...
 */
async function toDependencyTable(args, options) {
    const table = createDependencyTable();
    let mappedArgs = [];
    if (options.ownerChanged) {
        const newArgs = await getOwnerPerDependency(args, options);
        mappedArgs = _.cloneDeep(newArgs);
    } else {
        mappedArgs = _.cloneDeep(args);
    }
    const rows = Object.keys(mappedArgs.to).map(dep => {
        const from = mappedArgs.from[dep] || '';
