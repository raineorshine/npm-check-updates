...
function createDependencyTable(options) {
    options = options || {};
    var head = options.head || ['Dependency', 'Installed', 'Current', options.greatest ? 'Greatest' : 'Latest'];
    if(options.headColor) {
        head = head.map(function (col) { return chalk[options.headColor](col); });
    }
    return new Table({
        head: head,
...
function toDependencyTable(args, options) {
    ...
    options.head = [].concat([
        ...
        'Current',
        options.greatest ? 'Greatest' : 'Latest'
    ]);
    var table = createDependencyTable(options);
    var rows = Object.keys(args.upgraded).map(function (dep) {
        return [].concat([
            dep,
            args.installed ? args.installed[dep] : [],
            args.current[dep],
            args.upgraded[dep]
        ]);
    });
    // rows.forEach(cint.aritize(table.push.bind(table)), 1);
...
function printGlobalUpgrades(current, upgraded) {
    ...
    if (_.isEmpty(upgraded)) {
        ...
        for (var dep in upgraded) {
            print('"' + dep + '" can be updated from ' + current[dep] + ' to ' + upgraded[dep]);
        }
        if(options.errorLevel >= 2) {
            throw new Error('Dependencies not up-to-date');
        }
...
function printUpgrades(args) {
    ...
    if (_.isEmpty(args.upgraded)) {
        ...
        var unsatisfiedTable = toDependencyTable({
            ...
            upgraded: unsatisfiedUpgraded
        }, { headColor: 'blue', greatest: options.greatest });
        ...
        if(Object.keys(satisfiedUpgraded).length > 0) {
            ...
            var satisfiedTable = toDependencyTable({
                installed: args.installed,
                current: args.current,
                upgraded: satisfiedUpgraded
            }, { headColor: 'gray', greatest: options.greatest });
            print('\nThe following ' + (count === 1 ? 'dependency is' : 'depencencies are') + ' satisfied by ' + (count === 1 ? 'its' : 'their') + ' declared version range, but the installed version' + (count === 1 ? ' is' : 's are') + ' behind. ' + (count === 1 ? 'It' : 'They') + ' can be updated without modifying the package.json by running ' + chalk.blue('npm update') + ' or upgraded in the package.json by running ' + chalk.blue('ncu -ua') + '.\n')
            print(chalk.gray(satisfiedTable.toString()));
        }
    }
    print('');
}
