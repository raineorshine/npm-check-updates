...
function toDependencyTable(options, args) {
    ...
    const rows = Object.keys(args.to).map(function (dep) {
...
function analyzeGlobalPackages(options) {
    ...
    if (options.global && options.upgrade) {
        programError(options, `${chalk.cyan('ncu')} cannot upgrade global packages. Run ${chalk.cyan('npm install -g [package]')} to update a global package`);
    }
...
function analyzeProjectDependencies(options, pkgData, pkgFile) {
    ...
    return vm.upgradePackageDefinitions(current, options).spread(function (upgraded, latest) {
        ...
        const satisfied = cint.toObject(deps, function (dep) {
...
function printUpgrades(options, args) {
    ...
    // print table
    if (numUpgraded > 0) {
        const table = toDependencyTable(options, {
            from: args.current,
            to: upgraded
        }, {
