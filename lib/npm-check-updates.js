...
var UpgradeTable = require('./upgradetable');
...
// the symbol to be printed when a dependency is not found
var NOTFOUND = '--';
...
// =======
    ...
    var packageList = Object.keys(currentDependencies);

    // validate options.versionTarget
    var versionTarget = options.greatest ? 'greatest' : 'latest';

    if (logLevels[options.loglevel] >= logLevels.verbose) {
        print('Fetching ' + versionTarget + ' versions...');
    }

    // determine the getPackageVersion function from options.versionTarget
    switch (versionTarget) {
        case('latest'):
            getPackageVersion = vm.getLatestPackageVersion;
            break;
        case('greatest'):
            getPackageVersion = vm.getGreatestPackageVersion;
            break;
        default:
            var supportedVersionTargets = ['latest', 'greatest'];
            return Promise.reject('Unsupported versionTarget: ' + versionTarget +
    ...
    function getPackageVersionProtected(dep) {
        return getPackageVersion(dep)
            // update the upgradeTable
            .tap(function (version) {
                upgradeTable.update(dep, version);
            })
            .catch(function (err) {
                if (err.message == 404) { // eslint-disable-line eqeqeq
                    upgradeTable.update(dep, NOTFOUND);
                    return null;
                } else {
                    throw new Error(err);
                }
    ...
    return Promise.map(packageList, getPackageVersionProtected)
        ...
        // exclude null results (i.e. 404)
        .then(_.partialRight(_.pick, _.identity))
        // get upgraded versions based on current dependencies
        .then(function (latestVersions) {
            var upgradedDependencies = vm.upgradeDependencies(currentDependencies, latestVersions);
            return [upgradedDependencies, latestVersions];
        });
...
function analyzeGlobalPackages() {
    ...
    return vm.getInstalledPackages({packageManager: options.packageManager})
...
function analyzeProjectDependencies(pkgData, pkgFile) {
    ...
    upgradeTable = new UpgradeTable(current);

    if(options.logstyle === 'dynamic') {
        upgradeTable.start();
    }
    ...
    return Promise.all([
        ...
        pkgFile ? vm.getInstalledPackages({packageManager: options.packageManager}) : Promise.resolve(),
    ...
    .spread(function (current, installed, upgradedAndLatest) {
        ...
        if (options.json) {
            ...
            // replace with dynamic upgradeTable
            // printUpgrades({
            //     installed: installed,
            //     current: current,
            //     upgraded: upgraded,
...
function printUpgrades(args) {
    ...
    if (numUnsatisfied > 0) {
        ...
        if (args.pkgFile && !args.isUpgrade) {
            print('\nRun with ' + chalk.blue('-u') + ' to upgrade ' + getPackageFileName());
        }
    }
...
function programRunLocal() {
    ...
    if (logLevels[options.loglevel] >= logLevels.verbose) {
        ...
        print('Finding package file data...');
    }
    ...
    if (options.packageData) {
        ...
        if (logLevels[options.loglevel] >= logLevels.verbose) {
            print('Waiting for package data on stdin...');
        }
        pkgData = getstdin();
    } else {
