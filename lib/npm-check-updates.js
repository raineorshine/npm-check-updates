            ...
            if (pkgFile && !_.isEmpty(upgraded)) {
                if (options.upgrade) {
                    ...
                    writePackageFile(pkgFile, newPkgData)
                        .then(function () {
                            print('Upgraded ' + pkgFile + '\nUpdate the installed dependencies by running ' +
                                chalk.blue('npm update') + '\n');
                        });
                }
...
function printUpgrades(args) {
    ...
    if (_.isEmpty(args.upgraded)) {
        ...
        if (numSatisfied > 0) {
            ...
            print('The following dependenc' + (count === 1 ? 'y is' : 'ies are') + ' satisfied by ' +
                (count === 1 ? 'its' : 'their') + ' declared version range, but the installed version' +
                (count === 1 ? ' is' : 's are') + ' behind. You can install the latest version' +
                (count === 1 ? '' : 's') + ' without modifying your package.json by using ' + chalk.blue('npm update') +
                '. If you want to update the dependenc' + (count === 1 ? 'y' : 'ies') +
                ' in your package.json anyway, use ' + chalk.blue('ncu --upgradeAll') + '.\n');
...
function programInit() {
    ...
    if (options.global && options.upgrade) {
        print(chalk.blue('ncu') + ' cannot upgrade global packages. Run ' + chalk.blue('npm install -g [package]') +
...
function programRunLocal() {
    ...
    if (options.packageData) {
        ...
        // the following signals analyzeProjectDependencies to search for installed dependencies and to not print the
        // upgrade message
        pkgFile = null;
    } else {
