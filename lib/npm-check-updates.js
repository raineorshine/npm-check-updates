 ...
 */
async function printUpgrades(options, {current, upgraded, numUpgraded, total}) {
    print(options, '');

    if (vm.checkInternetConnection(options.noOnlineCheck)) {
        // check if there is an internet connection
        const isReachable = require('is-reachable');
        if (options.registry) {
            if (!await isReachable(options.registry)) {
                programError(options, '\nThe package repository is not reachable! Please check your internet connection.\n');
            }
        } else if (options.packageManager === 'bower') {
            if (!await isReachable('https://registry.bower.io')) {
                programError(options, '\nThe package repository is not reachable! Please check your internet connection.\n');
            }
        } else {
            if (!await isReachable('https://registry.npmjs.org')) {
                programError(options, '\nThe package repository is not reachable! Please check your internet connection.\n');
            }
        }
    }
