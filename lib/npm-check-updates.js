...
function analyzeProjectDependencies(options, pkgData, pkgFile) {
    ...
    return vm.upgradePackageDefinitions(current, options).then(async ([upgraded, latest]) => {
        ...
        if (options.json) {
            ...
            numUpgraded = printUpgrades(options, {
                current,
 ...
 * @param {Object} [args.latest] - The new versions (optional)
 * @param {number} [args.total] - The total number of all possible upgrades (optional)
 * @returns {number} - The number of upgraded packages
 */
function printUpgrades(options, {current, upgraded, latest, total}) {
