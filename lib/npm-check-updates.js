...
function printUpgrades(options, args) {
    ...
    var satisfied = cint.toObject(deps, dep => cint.keyValue(dep, vm.isSatisfied(args.latest[dep], args.current[dep])));

    var isSatisfied = _.propertyOf(satisfied);
    var satisfiedUpgraded = cint.filterObject(args.upgraded, dep => isSatisfied(dep) && (!args.latest || !args.installed || args.latest[dep] !== args.installed[dep]));
