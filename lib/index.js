...
function initOptions(options) {
  ...
  return {
    ...
    format,
    packageFile: options.deep ? `${deepPatternPrefix}${getPackageFileName(options)}` : options.packageFile
  }
}
...
async function run(options = {}) {
  ...
  async function getAnalysis() {
    ...
    if (options.cwd) {
      process.chdir(path.resolve(options.cwd.replace(/^~/, os.homedir())))
    }
    const pkgs = globby.sync(defaultPackageFilename, qlobbyOts)
    ...
    if (options.global) {
      ...
      analysis = await pkgs.reduce(async (previousPromise, packageFile) => {
        ...
        return {
          ...packages,
          [pkgFile]: await analyzeProjectDependencies(options, pkgData, pkgFile)
        }
      }, {})
      if (options.json) {
