  ...
  async function getAnalysis() {
    ...
    if (options.global) {
      ...
      analysis = await pkgs.reduce(async (previousPromise, packageFile) => {
        ...
        let opts = { ...options }
        opts.packageFile = packageFile
        const rcResult = getNcurc({ packageFile })
        if (rcResult && rcResult.config) {
          opts = _.merge(opts, rcResult.config)  
        }
          
        const [pkgData, pkgFile] = await findPackage(opts)
        return {
          ...packages,
          // index by relative path if cwd was specified
          [opts.cwd
            ? path.relative(path.resolve(opts.cwd), pkgFile)
              // convert Windows path to *nix path for consistency
              .replace(/\\/g, '/')
            : pkgFile
          ]: await analyzeProjectDependencies(opts, pkgData, pkgFile)
        }
      }, {})
      if (options.json) {
